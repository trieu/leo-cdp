...
public class RunSetupCoreDatabase {
	...
	static void setupCategoryData() {
		Category cate;
		List<String> catList = Arrays.asList("MARKET", "COMPANY", "PRODUCT", "FINISHED_PROJECT", "CUSTOMER_SERVICE",
				"DOCUMENT");
		Map<String, String> catDisplay = ImmutableMap.<String, String>builder().put("MARKET", "THỊ TRƯỜNG")
				.put("COMPANY", "CÔNG TY").put("PRODUCT", "SẢN PHẨM")
				.put("FINISHED_PROJECT", "CÔNG TRÌNH ĐÃ THỰC HIỆN").put("CUSTOMER_SERVICE", "DỊCH VỤ HẬU MÃI")
				.put("DOCUMENT", "TÀI LIỆU THAM KHẢO").build();
	...
	static void setupPageData(String catKeyword, String categoryKey) {
		...
		if ("MARKET".equals(catKeyword)) {
			...
			PageDaoUtil.save(new Page("Leo CDP", networkId, categoryKey, ownerId));

		} else if ("FINISHED_PROJECT".equals(catKeyword)) { // Query Filter Page
			PageDaoUtil.save(new Page("Các dự án thuộc Leo CDP", networkId, categoryKey, ownerId));

		} else if ("CUSTOMER_SERVICE".equals(catKeyword)) {
			// Query Filter Page
			PageDaoUtil.save(new Page("An toàn data", networkId, categoryKey, ownerId));
			PageDaoUtil.save(new Page("Bảo hành", networkId, categoryKey, ownerId));
			PageDaoUtil.save(new Page("Chính sách ban hang", networkId, categoryKey, ownerId));
