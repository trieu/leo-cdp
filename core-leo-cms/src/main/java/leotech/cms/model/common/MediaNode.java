...
public abstract class MediaNode implements PersistentArangoObject, Comparable<MediaNode> {
	...
	@Expose
	protected String contentClass = "";// class name. E.g: project, product,
										// item, youtube-video, facebook-video
										// ...
	...
	@Expose
	protected String headlineImageUrl = "";// default headline image about
											// content
	...
	// by user-generated
	@Expose
	protected List<String> topicKeys = new ArrayList<>(20);
	...
	public MediaNode(String id, String title, String mediaInfo, long networkId, int type, String pageId,
			String categoryKey, String ownerId) {
		if (id.startsWith(String.valueOf(networkId))) {
			this.id = id;
	...
	public MediaNode(String title, String mediaInfo, long networkId, int type, String pageId, String categoryKey,
			String ownerId) {
		initRequiredData(title, mediaInfo, networkId, type, ownerId);
		this.pageIds.add(pageId);
		this.categoryKeys.add(categoryKey);
	...
	public boolean isReadyForSave() {
		boolean ok = StringUtil.isNotEmpty(this.id) && networkId > 0 && StringUtil.isNotEmpty(this.slug)
	...
	public void compactDataForList(boolean headlineOnly) {
		...
		// this.title =
		// Base64.getEncoder().encodeToString(this.title.getBytes());
		// this.description =
