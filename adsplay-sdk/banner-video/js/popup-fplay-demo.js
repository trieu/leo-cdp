...
(function( $ ) {
    ...
    $.LightBoxAds = function( el, options ) {
        ...
        $.LightBoxAds.defaultOptions = {
          linkApi: "https://d4.adsplay.net/delivery?uuid="+ AdsPlayUUID.getUUID() +"&placement=1010&at=tvc&url=test-adsplay-demo-1380",
          ...
          $.ajax({
              ...
              success: function(data) {
                ...
                if ( linkMedia.indexOf( "youtu.be" ) >= 0 || linkMedia.indexOf( "youtube" ) >= 0 ) {
                  ...
                  var taga = $('<a href="'+ linkMedia +'" target="_blank"></a>');
                  linkMediaYouTuBe = linkMedia.split('=')[1];
                  var youtube = '<iframe id="player" class="' + base.options.classYoutubeAds + '" src="https://www.youtube.com/embed/'+ linkMediaYouTuBe +'?rel=0&autoplay=1&showinfo=0&controls=0&enablejsapi=1&version=3&loop=1&playlist='+ linkMediaYouTuBe +'"></iframe>';
                  var tempClass = divClass.append(youtube);
                  taga.append(tempClass);
                  base.$el.parent().append(taga);
                  ...
                  var player = new YT.Player('player', {
                    videoId: 'linkMediaYouTuBe', // YouTube Video ID
                    events: {
                      onReady: function(e) {
                        e.target.mute();
                      }
                  ...
                  var linkClick = 'https://youtu.be/iO1eGUptthA';
                    var divClass = $('<div class="'+ base.options.classVideoAds +'"></div>');
                    
                    var video = $( '<video class="video-js" id="'+ base.options.idVideoAds+'"  autoplay muted loop preload="auto"><source src="'+ linkMedia +'" type="video/mp4" /></video>' );
                    var taga = $('<a href="'+ linkClick +'" target="_blank"></a>');
        ...
        base.VideoAdDone = function() {
            // $(' .classClosePopup ').detach();
                ...
                // var x = $('.'+ base.options.classVideoAds ).detach();
                // $('.'+ base.options.classVideoAds ).attach();
                // $('body #test >a').prepend(x);
        ...
        base.onYouTubeIframeAPIReady = function() {
            var player;
            ...
            player = new YT.Player('player', {
              videoId: 'player', // YouTube Video ID
